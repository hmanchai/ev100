...
class Conversion:
    ...
    def __init__(self, rev, chip_version, py_log_path, py_log_name, pattern_category, vector_type,
        ...
        self.velocity_dft_cfg_path = velocity_dft_cfg_path
    ...
    #
    #     start = time.time()
    #     self.logger.info(f'**** File transfer starts for {pattern_category} {vector_type} ****')
    #     self.logger.info(f'src dir: {dir_orig}, dest dir: {local_dir} ')
    #     try:
    #         shutil.copytree(dir_orig, local_dir)
    #     except FileExistsError:
    #         self.logger.exception(f'Error! Please check the content in this existing path :{dir_orig}.')
    #     else:
    #         end = time.time()
    #         elapsed = end - start
    ...
    #     start = time.time()
    #     self.logger.info(f'**** File transfer starts for {pattern_category} {vector_type}: src:{dir_orig}  dest:{local_dir} ****')
            ...
            for file in files:
                ...
                if fnmatch.fnmatch(file, '*.stil.gz'):
                    ...
                    try:
                        ...
                        self.convert_all_pats(root, pattern_category, vector_type, log_name, enable_del_zip)
                    except Exception:
    ...
    def convert_all_pats(self, path_stil_files, pattern_category, vector_type, log_name, enable_del_zip):
        ...
        # unzip STIL files, func has internal self.logger
        unzip_result = self.unzip_n_rename(path_stil_files)
        if not unzip_result:
            self.logger.error('Error! Unzipping fails, so no further steps will be executed.')
        else:
            # copy Velocity CFG file to the specified dir
            self.copy_cfg(path_stil_files, pattern_category)
            ...
            if not conv_result:
                ...
                try:
                    ...
                    cyc_cnt = self.extract_cycle_count(path_stil_files)
    ...
    def temp_recompile(self, path_stil_files):
        ...
        try:
            ...
            self.logger.exception('Error! Pingroup removal and re-compilation actions not finished completely.')
        else:
            if not compile_err:  # in case compilation fails but no traceback error returned
    ...
    def unzip_n_rename(self, path_stil_files):
        ...
        try:
            ...
            list_zip_files_sorted = self.sorted_alphanumeric(list_zip_files)
            if not list_zip_files:
                raise FileNotFoundError("No .gz zip file exists in this directory: {}\n".format(path_stil_files))
            self.logger.info(f'{len(list_zip_files_sorted)} .gz zip files exist in the directory : {path_stil_files}')
    ...
    def copy_cfg(self, dest_dir, pattern_category):
        """
    ...
    def convert_stil_files(self, path_stil_files, pattern_category):
        """
        ...
        if os.path.exists(cfg):
            ...
            if not list_stil_files:
                ...
                # sort list_stil_path alphanumerically
                list_stil_files_sorted = self.sorted_alphanumeric(list_stil_files)
                ...
                self.logger.info('Starting to convert STIL to DP .....')
                start = time.time()
                conv_result = os.system(conv_bat)
                self.logger.info(f'conversion_result: {conv_result}')
                end = time.time()
                elapsed = end - start
                self.logger.info(f'== Time elapsed for converting STIL to DP: {timedelta(seconds=elapsed)} ==')
                ...
                if len(list_dp_files) == (len(list_stil_files) + 1):
                    ...
                    self.logger.info('Cool! Conversion Successful')
                    return 1
                else:
                    # print('\n****** Conversion NOT Successful ******\n')
                    self.logger.error('Error! Conversion NOT Successful')
                    return 0
    ...
    def get_timing(self, path_stil_files, change_timing=False):
        """
            ...
            if len(h_file_list) == 1:
                ...
                with open(h_file, 'r') as f:
                    ...
                    if match:
                        ...
                        self.logger.info('Initial scan clock period is {}ns'.format(period_initial))
                ...
                # self.logger.error(f'Error! {len(h_file_list)} .h files are found')
    ...
    def change_timing(self, path_stil_files, period_new):
        ...
        """
        try:
            h_file, list_lines, index_line_period, period_initial = self.get_timing(path_stil_files, change_timing=True)
    ...
    def compile_do_files(self, path_stil_files):
        """
        ...
        for dp_file in dp_file_list:
            ...
            if kw in dp_file:
                ...
                self.logger.debug('Compilation command:\n{}\n'.format(compile_cmd))

                self.logger.info('Starting to compile DO .....')
                start = time.time()
                err = os.system(compile_cmd)
                # print('Compilation error: {}\n'.format(err))
                if err:
                    # print('****** Compilation Has Error! ******\n')
                    self.logger.error('Error! Compilation NOT Successful.')
                else:
                    # print('****** Compilation Completed ******\n')
                    self.logger.info('Cool! Compilation Successful.')
                end = time.time()
                elapsed = end - start
                self.logger.info(f'== Time elapsed for compiling DO: {timedelta(seconds=elapsed)} ==')
    ...
    def patch_timesets_header(self, path_stil_files, pattern_category):
        """
        ...
        if os.path.exists(path_timesets):
            ...
            with open(self.patch_timesets_path, 'r') as f:
    ...
    def log_conversion_info(self, path_stil_files, pattern_category, vector_type, compile_error, do_file_base,
        ...
        if log_name == 0:
            # mode = 'a'
            path_all_csv = os.path.join(self.conversion_log_csv_path, '*csv')
    ...
    def del_zip(self, path_stil_files):
        """
        ...
        list_zip_files = glob.glob(zip_file_path)
        if not list_zip_files:
            self.logger.warning(f'Warning! No stil.gz zip file exists in this directory: {path_stil_files}')
            # raise FileNotFoundError("No .gz zip file exists in this directory: {}\n".format(path_stil_files))
        else:
            ...
            for zip_file in list_zip_files:
                try:
                    ...
                    self.logger.exception('Error in deleting zip files.')
                else:
